...
RSpec.describe "download balances as xlsx" do
  ...
  it "works and has the correct content" do
    ...
    expect(wb.cell(1, 2)).to eq 'Kreditvertrag ID'
    expect(wb.cell(1, 3)).to eq 'Kreditvertrag Nr'
    expect(wb.cell(1, 4)).to eq 'Datum'
    expect(wb.cell(1, 5)).to eq 'KreditgeberIn'
    ...
    expect(wb.cell(2, 2)).to eq @balance_1.credit_agreement.id
    expect(wb.cell(2, 3)).to eq @balance_1.credit_agreement.number.to_i
    expect(wb.cell(2, 4)).to eq @balance_1.date
    expect(wb.cell(2, 5)).to eq [@balance_1.creditor.name, @balance_1.creditor.first_name].join(', ')
    ...
    expect(wb.cell(3, 2)).to eq @balance_2.credit_agreement.id
    expect(wb.cell(3, 3)).to eq @balance_2.credit_agreement.number.to_i
    expect(wb.cell(3, 4)).to eq @balance_2.date
    expect(wb.cell(3, 5)).to eq [@balance_2.creditor.name, @balance_2.creditor.first_name].join(', ')
    ...
    expect(wb.cell(4, 2)).to eq @balance_3.credit_agreement.id
    expect(wb.cell(4, 3)).to eq @balance_3.credit_agreement.number.to_i
    expect(wb.cell(4, 4)).to eq @balance_3.date
    expect(wb.cell(4, 5)).to eq [@balance_3.creditor.name, @balance_3.creditor.first_name].join(', ')
