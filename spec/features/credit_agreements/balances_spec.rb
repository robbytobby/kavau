    ...
    context "- as #{type} -" do
      ...
      context "on the index page" do
        before :each do
          ...
          @manual_balance = create :balance, :manual
        end

        it "all balances are shown" do
          visit balances_path
          @balances.each do |bal|
            expect(page).to have_css("tr#balance_#{bal.id}")
            within "tr#balance_#{bal.id}" do
              ...
              expect(page).to have_css("a#edit_balance_#{bal.id}")
              expect(page).not_to have_css("a#delete_balance_#{bal.id}")
            end
          end
          within "tr#balance_#{@manual_balance.id}" do
            expect(page).to have_css("a#delete_balance_#{@manual_balance.id}")
          end
        end
      end
    end
