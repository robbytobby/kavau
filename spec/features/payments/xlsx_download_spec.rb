...
RSpec.describe "download payments as xlsx" do
  ...
  it "works and has the correct content" do
    ...
    expect(wb.cell(1, 6)).to eq 'Konto'
    expect(wb.cell(1, 7)).to eq 'Betrag [€]'
    ...
    expect(wb.cell(2, 6)).to eq @payment_1.credit_agreement.account.name
    expect(wb.cell(2, 7)).to eq @payment_1.amount
    ...
    expect(wb.cell(4, 6)).to eq @payment_2.credit_agreement.account.name
    expect(wb.cell(4, 7)).to eq @payment_2.amount * @payment_2.sign
  end
end
